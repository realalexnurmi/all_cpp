...
void	Bureaucrat::execute(Form &form){
	...
	if (i == 1)
		std::cout<< _name <<" SUCCESS! "<<form.getName() << std::endl;
	else if (i == -1)
		std::cout<< _name <<" ERROR; Need grade "<<form.getExecutionGrade() << std::endl;
	else if (i == 0)
		std::cout<< _name << " ERROR; Form not signed. "<<form.getName()<< std::endl;
}
