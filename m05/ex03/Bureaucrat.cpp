...
/*   Updated: 2022/01/27 05:03:32 by enena            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void				Bureaucrat::executeForm(const Form& toExec) const
{
	if (toExec.isSigned())
	{
		if (this->getGrade() <= toExec.getGradeExecute())
		{
			std::cout << this->getName() << " executes "
				<< toExec.getName() << std::endl;
			toExec.action();
		}
		else
			std::cout << this->getName() << " cannot execute "
				<< toExec.getName() << "because of have low grade" << std::endl;
	}
	else
		std::cout << toExec.getName() << " is not signed." << std::endl;
}
